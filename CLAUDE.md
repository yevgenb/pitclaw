...
firmware/
  ...
  platformio.ini
enclosure/
  fusion-r13/                   # Current controller Fusion model, STL/STEP files, coupons and checks
...
Use `enclosure/fusion-r13/README.md` and the editable
...
bezel keys remain 12 × 2.1 mm. All sixteen insert pilots are Ø4.2 mm; M3 screw
passages remain Ø3.4 mm. Four internal M3 mounting
inserts on a 72 × 74 mm pattern accept future mounts from underneath.

The Fusion archive is the editable CAD source. `export_r13.py` exports its
current parts and previews; `reference/carrier-case.scad` is the independent
OpenSCAD model regenerated by `reference/export_reference.py --copy-coupons`
and checked by `reference/verify_reference.py`. `verify_exports.py`
...
Use the r13 guide for orientations, screw lengths, insert installation, support
