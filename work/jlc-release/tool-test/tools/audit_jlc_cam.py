...
with tempfile.TemporaryDirectory(prefix='pitclaw-cam-audit-') as temp:
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        for plated,layer in [(True,stack.drill_pth),(False,stack.drill_npth)]:
            expected=Counter(key(h['diameter_mm'],h['endpoints_mm']) for h in ref['holes'] if h['plated']==plated)
            actual=Counter(holekey(o) for o in layer.objects)
            assert expected==actual,('Excellon/native hole mismatch',plated,expected-actual,actual-expected)
        outline=[]
        ...
        parser_warnings=sorted(set(str(w.message).split(': ',1)[-1] for w in caught))
        # KiCad puts G90 after M95; Gerbonara accepts its unambiguous absolute coordinates.
        assert all('G90 header statement found after end of header' in w for w in parser_warnings),parser_warnings
