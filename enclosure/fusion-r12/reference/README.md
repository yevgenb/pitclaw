# Independent r12 enclosure reference with future-mount inserts

**Status: the corrected reference and matching native Fusion model pass their geometry checks. Physical fit remains to be confirmed with the updated coupon.**
...
The case includes four interior mounting bosses and external screw access holes. The
...
## Corrected full-depth fascia guides

The fascia runner and receiver now have the same cross-section throughout
Y−52…−43, a 9 mm guide length. This corrects the CAD mismatch between the former
thicker front segment and the open inner groove. The receiver has a continuous
45° retaining roof and the runner has a matching sloped face. There is no flare,
front-only cover, or change of clearance along the depth.

The positive-X profiles below are mirrored on the other side:

- Runner XZ vertices: (32.7,3.8), (36,3.8), (36,5.8), (32.7,9.1).
- Receiver cavity XZ vertices: (32.7,3.8), (36.4,3.8), (36.4,5.9), (32.7,9.6).
- Outer side clearance: **0.4 mm**.
- Sloping roof clearance: **0.5 mm vertically**, or **0.354 mm normal to the face**.
- Bearing surface: **Z3.8**, intentional contact. Rear stop: **Y−43**, unchanged.

The side web reaches Z9.5 and joins the runner throughout its length. Receiver
material is clipped to the original rounded case outline. Its main inner roof
is 1.6 mm thick, extending to Z10.6. The roof ends at Y−43, leaving the mounting
insert/tool region clear. The 45° roof also avoids an inaccessible horizontal
support ledge when the bottom is printed in its existing orientation.

`verify_guide_sections.py` independently measures the exported solids at the
outer section **Y−51.8** and inner section **Y−47**. Both sections must have these
same dimensions on both sides:

| Absolute X, mm | Runner top Z, mm | Receiver roof Z, mm | Vertical gap, mm |
|---|---|---|---|
| 33.5 | 8.3 | 8.8 | 0.5 |
| 34.0 | 7.8 | 8.3 | 0.5 |
| 35.0 | 6.8 | 7.3 | 0.5 |
...
## Current lid retention geometry
...
- The constant guide profiles above support and capture the fascia along its
  full insertion depth. The lid keys separately prevent outward withdrawal.
...
`verify_reference.py` runs 31 declared checks: the existing 26 fit/assembly,
mounting-screw, and tool-access scenarios, plus five new guide-capture checks.
At Y−51.8, −50.8, −49, −47, and −44, both actual local runners translated upward
by 0.7 mm must intersect the receiver roofs with the lid absent. Each side and
each depth is required to show positive capture; the old inner groove failed
this behavior. These checks prove geometric blocking, not a retention-force rating. The production case
...
top meets the Z27.6 seam. The approached endpoint must be an empty intersection.
...
The coupon still requires inspection in the slicer, particularly its probe pocket
and lid-keeper bridges. The key chamfer is an assembly lead-in, not a substitute for
