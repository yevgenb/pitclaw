#!/usr/bin/env python3
"""Check the isolated r12 interior-mount-insert handedness reference and its installation scenarios.

Run with Python 3 plus trimesh, for example:
  python3 enclosure/fusion-r12/reference/verify_reference.py
...
SOURCES = (
    "enclosure/fusion-r12/reference/carrier-case.scad",
    "enclosure/fusion-r12/reference/carrier-interface.scad",
...
EXPECTED_SIZE = {
    ...
    "retainer": [80, 96, 2.5],
    "fascia": [72,27.3,9.5],
    "joint_bottom": [33,11.5,27.6],
    "joint_top": [33,11.5,21.9],
    "joint_fascia": [26,27.3,9.5],
    "coupon": [86, 22, 27.6],
    "insert_coupon": [36, 16, 7.5],
    "mount_coupon": [16,21,10],
}
MESH_NAMES = {
    "top": "carrier-top.stl",
    "bottom": "carrier-bottom.stl",
    "retainer": "carrier-retainer.stl",
    "fascia": "probe-fascia.stl",
    "joint_bottom": "joint-coupon-bottom.stl",
    "joint_top": "joint-coupon-top.stl",
    "joint_fascia": "joint-coupon-fascia.stl",
    "coupon": "fit-coupon.stl",
    "insert_coupon": "insert-coupon.stl",
    "mount_coupon": "mount-coupon.stl",
}
...
def scenarios() -> list[Check]:
    checks = [Check("nominal-" + part, part, (1.6, 1.6, 3, 0, 0)) for part in (
        ...
        "shell-closure-check", "fascia-fit-check", "fascia-insertion-check", "lid-closing-check", "fascia-lift-check", "mount-screw-check", "mount-tool-check")]
    for part in ("guide-capture-front-check", "guide-capture-early-check",
    ...
    for label, stack in (("low", (1.44, 1.44, 2.9)), ("high", (1.76, 1.76, 3.1))):
        for x in (-0.2, 0.2):
            for part in ("carrier-installation-check", "electronics-closure-check", "fascia-fit-check", "fascia-insertion-check"):
...
def run_check(check: Check, command: list[str], output: Path, timeout: float) -> dict:
    ...
    args.append(str(Path(__file__).with_name("carrier-case.scad")))
...
def main() -> int:
    ...
    report = {
        "status": "RUNNING", "revision": "carrier-enclosure-fusion-r12-reference",
        ...
        "assumptions": [
            ...
            "Guide capture checks require positive intersections on both sides; shell closure may retain only coplanar Z27.6 contact; other intersections must be empty.",
