...
else if(part=="joint-coupon-bottom") translate([-26.5,46.25,0]) joint_coupon_bottom();
else if(part=="joint-coupon-top") translate([-26.5,-46.25,case_height]) rotate([180,0,0]) joint_coupon_top();
else if(part=="joint-coupon-fascia") translate([-23,13.65,52]) rotate([90,0,0]) joint_coupon_fascia();
...
module probe_fascia() {
    difference() {
        union() {
            intersection() {
                probe_pocket_blank();
                box_bounds(-33,33,-53,-42.5,0,27.3);
            }
            for(s=[-1,1]) scale([s,1,1]) {
                box_bounds(31,33,-52,-42.5,2.48,9.5);
                guide_male();
        ...
        probe_pocket_cut();
        for(x=[-21,21])
            box_bounds(x-6.3,x+6.3,-50.85,-48.25,20.78,27.32);
    }
}
...
module joint_coupon_bottom() {
    intersection() { bottom_shell(); box_bounds(10,43,-52,-40.5,-.02,60); }
}
module joint_coupon_top() {
    intersection() { top_bezel(); box_bounds(10,43,-52,-40.5,-.02,60); }
}
module joint_coupon_fascia() {
    intersection() { probe_fascia(); box_bounds(10,43,-52,-40.5,-.02,60); }
}

module fascia_lift_collision() {
    // Approach the0.3mm upward stop within0.0001mm numerical margin.
    // At exactly0.3mm, the fascia top intentionally contacts seamZ27.6.
    // Tiny numerical trimming excludes only deliberate Y-43 guide end contact.
    intersection() {
        translate([0,0,.2999]) probe_fascia();
        union() {
            intersection() {
                bottom_shell();
                union() {
