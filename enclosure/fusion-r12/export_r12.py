"""Export the four printed r12 parts and illustrate the assembly sequence."""
...
def run(_context):
    ...
    if d.snapshots.hasPendingSnapshot:d.snapshots.add().name='Restore assembled r12 positions'
    ...
    for i,o in enumerate(occ):o.isLightBulbOn=i in [0,8]
    camera(app,(12,16,24),(0,0,1.0))
    assert app.activeViewport.saveAsImageFile(str(OUT/'bottom-mounts-inside.png'),1600,1200)
    cam=app.activeViewport.camera;cam.cameraType=C.CameraTypes.OrthographicCameraType
    cam.eye=C.Point3D.create(0,0,25);cam.target=C.Point3D.create(0,0,0)
    ...
    camera(app,(9,-20,5))
    runpy.run_path(str(OUT/'check_native_r12.py'))['run'](_context)
    target=OUT/'pitclaw-enclosure-r12.f3d'
