#!/usr/bin/env python3
"""Check the bounded fit-test candidate and its clamped comparison coupons.

Run with Python 3 plus trimesh, for example:
  python3 enclosure/fusion-r12/fit-test/verify_fit_test.py
...
SOURCES = (
    "enclosure/fusion-r12/fit-test/sources/candidate.scad",
    "enclosure/fusion-r12/fit-test/sources/control.scad",
    "enclosure/fusion-r12/fit-test/sources/carrier-interface.scad",
...
def scenarios() -> list[Check]:
    checks = [Check("nominal-" + part, part, (1.6, 1.6, 3, 0, 0)) for part in (
        ...
        "shell-closure-check", "fascia-fit-check", "fascia-insertion-check", "lid-closing-check", "fascia-lift-check", "mount-screw-check", "mount-tool-check", "shell-closure-with-fascia-check")]
...
def run_check(check: Check, command: list[str], output: Path, timeout: float) -> dict:
    ...
    args.append(str(Path(__file__).with_name("sources")/"candidate.scad"))
...
def main() -> int:
    ...
    report = {
        "status": "RUNNING", "revision": "bounded-clamped-fit-test",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "sources_sha256": sources_before,
        "verifier_sha256": sha256(Path(__file__)),
        "scope": "FIT TEST ONLY: candidate full-assembly geometry and five cropped comparison meshes; released enclosure untouched.",
        "assumptions": [
