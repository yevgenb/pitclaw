...
def run(_context):
    ...
    report['allowed_fascia_lift_0_1999mm_clash_volume_mm3']=overlap(fascia,top,dz=.1999)
    assert report['allowed_fascia_lift_0_1999mm_clash_volume_mm3']<1e-5
    path=[]
    ...
    # Each guide depth must provide its own upper capture without the lid.
    # The previous front-cover/open-groove design fails the inner stations.
    guide_capture=[]
    for side in (-1,1):
        for y in (-51.8,-50,-48.5,-47,-43.5):
            section=manager.copy(fascia)
            bounds=C.OrientedBoundingBox3D.create(C.Point3D.create(side*3.47,y/10,.685),
                C.Vector3D.create(1,0,0),C.Vector3D.create(0,1,0),.27,.02,.63)
            cutter=manager.createBox(bounds)
            assert manager.booleanOperation(section,cutter,F.BooleanTypes.IntersectionBooleanType)
            assert section.faces.count and section.isSolid and section.volume>0
            nominal=overlap(section,bottom)
            running=overlap(section,bottom,dz=.3)
            blocked=overlap(section,bottom,dz=.7)
            assert nominal<1e-5 and running<1e-5 and blocked>1e-5,(side,y,nominal,running,blocked)
            guide_capture.append({'side':side,'depth_y_mm':y,'nominal_clash_mm3':nominal,
                'up_0_3mm_clash_mm3':running,'up_0_7mm_blocked_volume_mm3':blocked})
    report['full_depth_guide_capture_without_lid']=guide_capture
    support=[]
    for side in (-1,1):
        for y in (-49.6,-43.5):
            section=manager.copy(fascia)
            bounds=C.OrientedBoundingBox3D.create(C.Point3D.create(side*3.47,y/10,.685),
                C.Vector3D.create(1,0,0),C.Vector3D.create(0,1,0),.27,.04,.63)
            assert manager.booleanOperation(section,manager.createBox(bounds),F.BooleanTypes.IntersectionBooleanType)
            for drop in (.05,.10):
                v=overlap(section,bottom,dz=-drop)
                assert abs(v-2.65*.4*drop)<.003,(side,y,drop,v)
                support.append({'side':side,'y_mm':y,'drop_mm':drop,'contact_volume_mm3':v})
    report['two_seating_lands_contact_checks']=support
    ...
    for x in (-36,36):
        for y in (-37,37):
            body=manager.createCylinderOrCone(C.Point3D.create(x/10,y/10,.35),.2,
                C.Point3D.create(x/10,y/10,.75),.2)
            bore=manager.createCylinderOrCone(C.Point3D.create(x/10,y/10,.349),.125,
                C.Point3D.create(x/10,y/10,.751),.125)
            assert manager.booleanOperation(body,bore,F.BooleanTypes.DifferenceBooleanType)
            nominal=overlap(body,floor);blocked=overlap(body,floor,dz=-1.5)
            assert nominal<1e-5 and blocked>1e-5
            minimum_capture.append({'centre_mm':[x,y],'minimum_insert_od_mm':4.0,
                'nominal_floor_clash_mm3':nominal,'outward_1_5mm_floor_contact_mm3':blocked})
    report['minimum_od_secondary_capture_checks']=minimum_capture
