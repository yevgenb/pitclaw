#!/usr/bin/env python3
"""Measure full-part running gaps, ramps and support lands on both guides."""
...
def main():
    files=['carrier-bottom.stl','probe-fascia.stl','carrier-case.scad']
    report={'status':'RUNNING','sources_sha256':{n:sha(ROOT/n) for n in files},
        'verifier_sha256':sha(Path(__file__)),'scope':'Actual exported full-part sections on both guides; nominal assembly geometry is checked separately.','sections':[]}
    try:
        bottom=trimesh.load(ROOT/'carrier-bottom.stl',force='mesh')
        fascia=trimesh.load(ROOT/'probe-fascia.stl',force='mesh')
        fascia.apply_transform([[1,0,0,0],[0,0,1,-52],[0,-1,0,0],[0,0,0,1]])
        for label,y,expected_floor in [('front relief',-51.5,3.5),('front ramp',-50.3,3.65),('front land',-49.6,3.8),('falling ramp',-48.9,3.65),('middle relief',-47,3.5),('rear ramp',-44.3,3.65),('rear land',-43.5,3.8)]:
            ...
            rows=[]
            for x in [-35.8,-35,-34,-33.5,33.5,34,35,35.8]:
                ...
                near(male_bottom,3.8);near(male_top,41.8-abs(x));near(floor,expected_floor)
            ...
            side_gaps=[]
            for sign in [-1,1]:
                male_edge=min(sign*x for x in crossings(f,2,4.5,0) if sign*x>33.3)
                wall=min(sign*x for x in crossings(b,2,4.5,0) if sign*x>33.3)
                near(male_edge,36);near(wall,36.3);near(wall-male_edge,.3)
                side_gaps.append(wall-male_edge)
            report['sections'].append({'label':label,'y_mm':y,'samples':rows,'left_right_side_gap_mm':side_gaps})
