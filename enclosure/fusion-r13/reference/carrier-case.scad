// PitClaw Rev B carrier enclosure r13: bottom, top, removable fascia and internal retainer.
...
// User M3 insert: measured OD4.0..5.0; L4.0 assumed; accepted pilot4.2.
...
insert_length = 4;
insert_pilot = 4.2; // accepted pilot for user inserts
insert_bore_depth = 5;
insert_boss_od = 9;
carrier_standoff = 5;
...
module_side_clearance = 0.6;      // body tolerance plus print allowance; no glass pinch
...
retainer_tab_od = 6.4;
retainer_anchor_xy = [[-35.5,-40],[35.5,-40],[-35.5,40],[35.5,40]];
...
power_profile_clearance = 0.5;
carrier_tail_projection = 3;     // maximum clipped lead/solder depth; inspect build
probe_axis_above_pcb = 2.55;      // 5.10 mm housing/bore center; verify sample seating
probe_body_height = 5.1;
...
usb_pocket_depth = 2;
usb_pocket_z_offset = 0.5;       // stronger web above carrier channel; socket stays fixed
usb_inward_backing = 1;
usb_socket_opening = [10.14,4.7]; // fitted shell, thickness/spacer allowance + seat lift
...
usb_relief_z_clearance = 0.8;   // separate Z budget; preserve thin blind-pocket Y skins
carrier_channel_xy_clearance = 0.5;
carrier_channel_z_clearance = 0.5;

// Diagnostic witness inputs ONLY; changing these must not resize the printed case.
check_pcb_thickness = 1.6;
check_pd_thickness = 1.6;
check_pd_spacer_height = 3;
check_board_edge_growth = 0;
check_module_shift_x = 0;
...
usb_pocket_channel_gap = carrier_top_z+usb_axis_above_pcb+usb_pocket_z_offset-usb_pocket_size[1]/2-
    (carrier_top_z+carrier_channel_z_clearance+carrier_slide_lift);
...
assert(usb_pocket_channel_gap >= 0.8-eps, "USB pocket/channel web is too thin");
...
echo("FIT_CHECK_INPUTS", [check_pcb_thickness, check_pd_thickness,
     check_pd_spacer_height, check_board_edge_growth, check_module_shift_x]);
...
module connector_openings() {
    ...
    translate([carrier_power_x[2]-30,case_length/2,
        ...
        translate([0,eps,usb_pocket_z_offset]) rotate([90,0,0])
...
module usb_pocket_backing() {
    ...
    translate([carrier_power_x[2]-30,inside_length/2+eps,
               carrier_top_z+usb_axis_above_pcb+usb_pocket_z_offset]) rotate([90,0,0])
...
module carrier_blind_reliefs() {
    ...
    // 0.5 mm end clearance leaves 1.0 mm continuous exterior plastic.
    translate([-30-carrier_channel_xy_clearance,inside_length/2-eps,
               carrier_bottom_z-carrier_channel_z_clearance])
        cube([60+2*carrier_channel_xy_clearance,
              46+carrier_case_offset_y+carrier_channel_xy_clearance-inside_length/2+eps,
              carrier_board_mm[2]+2*carrier_channel_z_clearance+carrier_slide_lift]);
    // These clearances are hidden behind the recessed USB floor. Include the
    // 0.3 mm installation lift. Z is enlarged independently of the thin Y skin.
    translate([carrier_power_x[2]-30-pd_pcb_size[0]/2-blind_pocket_clearance,
               usb_backing_inner_y-eps,
               carrier_top_z+pd_spacer_height-usb_relief_z_clearance])
        cube([pd_pcb_size[0]+2*blind_pocket_clearance,
              pd_pcb_front_local_y+blind_pocket_clearance-usb_backing_inner_y+eps,
              pd_pcb_size[2]+2*usb_relief_z_clearance+carrier_slide_lift]);
    // Continuous round spacer/head clearances preserve material in Y.
    // Nuts lie below the backing and need no cuts. Minimum hardware-pocket skin is
    // 0.983 mm; these pockets cannot open onto the visible access-pocket floor.
    for(p=pd_mount_xy)
        translate(concat(carrier_local_xy(p),
                         [carrier_top_z-usb_relief_z_clearance]))
            cylinder(d=pd_hardware_diameter+2*blind_pocket_clearance,
                     h=pd_spacer_height+pd_pcb_size[2]+pd_hardware_head_height+
                       carrier_slide_lift+2*usb_relief_z_clearance);
}
...
module bottom_shell_board_coordinates() {
    difference() {
        union() {
            difference() {
...
module carrier_sweep_board_coordinates(offset0,offset1,lift0,lift1) {
    checked_top = carrier_bottom_z+check_pcb_thickness;
    checked_pd_base = checked_top+check_pd_spacer_height;
    checked_usb_center = checked_pd_base+check_pd_thickness+
                         usb_axis_above_pcb-pd_spacer_height-pd_pcb_size[2];
    // Ignore only the intentional zero-clearance seating contact with posts.
    swept_carrier_box([-30-check_board_edge_growth,-46-check_board_edge_growth,carrier_bottom_z+eps],
        [60+2*check_board_edge_growth,92+2*check_board_edge_growth,check_pcb_thickness-eps],
        offset0,offset1,lift0,lift1);
    assert(!is_undef(carrier_component_boxes) && len(carrier_component_boxes)==31,
           "Export all placed component envelopes before checking case fit");
    for(b=carrier_component_boxes)
        swept_carrier_box([b[0]-30,b[1]-46,checked_top],
            [b[2],b[3],b[4]],offset0,offset1,lift0,lift1);
    ...
    for(i=[0:2]) {
        ...
        swept_carrier_box([carrier_power_x[i]-30-power_face_size[i][0]/2+
                          (i==2 ? check_module_shift_x : 0),
            carrier_power_face_y[i]-46-depth,
            (i==2 ? checked_usb_center : checked_top+power_face_center_z[i])-
            power_face_size[i][1]/2],
            [power_face_size[i][0],depth,power_face_size[i][1]],
            offset0,offset1,lift0,lift1);
    }
    for(x=carrier_probe_x) {
        swept_carrier_box([x-30-2.55,-46,checked_top],
                         [5.1,8,5.1],offset0,offset1,lift0,lift1);
        hull() for(v=[[offset0,lift0],[offset1,lift1]])
            translate([x-30,carrier_probe_face_y-46+v[0],checked_top+2.55+v[1]])
                rotate([-90,0,0]) cylinder(d=4,h=3.1);
    }
    swept_carrier_box([carrier_power_x[2]-30-pd_pcb_size[0]/2+check_module_shift_x,
        pd_pcb_front_y-46-pd_pcb_size[1],checked_pd_base],
        [pd_pcb_size[0],pd_pcb_size[1],check_pd_thickness],offset0,offset1,lift0,lift1);
    for(p=pd_mount_xy) {
        ...
        // Include the actual spacer envelope; it previously intersected the
        // pocket backing unnoticed. Carrier bolts locate the spacer centers.
        swept_carrier_cylinder([p[0]-30,p[1]-46,checked_top],pd_hardware_diameter,
            check_pd_spacer_height,offset0,offset1,lift0,lift1);
        swept_carrier_cylinder([p[0]-30,p[1]-46,
...
module carrier_witness_board_coordinates() {
    translate([0,carrier_case_offset_y,0]) {
        ...
        color([0.2,0.22,0.25,0.18])
            for(b=carrier_component_boxes)
                translate([b[0]-30,b[1]-46,carrier_top_z]) cube([b[2],b[3],b[4]]);
        if(!is_undef(carrier_relay_box))
        ...
        for(x=carrier_probe_x)
            color([0.18,0.18,0.19])
                translate([x-30,carrier_probe_face_y-46+3,
                           carrier_top_z+probe_axis_above_pcb])
                    rotate([90,0,0]) cylinder(d=probe_body_height,h=6,center=true);
        for(i=[0:1]) {
...
module assembly(explode=0) {
    ...
    color([0.30,0.34,0.38]) probe_fascia();
...
module electronics_closure_collision() {
    ...
    assert(glass_front_z-display_rear_depth-(carrier_bottom_z+check_pcb_thickness)-carrier_relay_box[4]
           >= 2.64-eps, "K1 violates its thickness-toleranced local rear clearance");
    ...
    intersection() {
        ...
        union() {
            ...
            // Installed carrier screws/insulating washers: Ø7 x3.5 mm reserve.
            for(p=carrier_mount_xy)
                translate(concat(carrier_local_xy(p),[carrier_bottom_z+check_pcb_thickness]))
                    cylinder(d=7,h=3.5);
        }
    }
}
...
module fit_coupon() {
    // Exact power-end section of the bottom, printed in the same orientation.
    // Check actual plugs, Ø5 spacers and the populated carrier's slide locally.
    // The full carrier hangs beyond the coupon; support it level during a test.
    translate([0,-41,0]) intersection() {
        bottom_shell();
        translate([-case_width,30,-eps]) cube([2*case_width,22+eps,seam_z+2*eps]);
    }
}

module insert_coupon() {
    // Same Ø9 bosses, 5 mm blind pilot depth and 2.5 mm closed floor.
    // Engraved labels identify 4.0 / 4.1 / 4.2 mm bores, left to right.
    difference() {
        union() {
            translate([-18,-8,0]) cube([36,16,2.5]);
            for(x=[-12,0,12]) translate([x,1,2.5-eps]) cylinder(d=9,h=5+eps);
        }
        for(i=[0:2]) {
            translate([-12+i*12,1,2.5]) cylinder(d=4+i*0.1,h=5+eps);
            translate([-12+i*12,-5.3,2.1]) linear_extrude(height=.5)
                text(str(4+i*0.1),size=2.2,halign="center",valign="center");
        }
    }
}

module shell_closure_collision() {
    // Intentional meeting faces have zero volume; positive volume is a clash.
    intersection() {
        bottom_shell();
        union() {
            top_bezel();
...
if(part=="guide-capture-front-check") guide_capture_collision(-51.8);
else if(part=="guide-capture-early-check") guide_capture_collision(-50.8);
else if(part=="guide-capture-midfront-check") guide_capture_collision(-49);
else if(part=="guide-capture-middle-check") guide_capture_collision(-47);
else if(part=="guide-capture-rear-check") guide_capture_collision(-44);
else if(part=="mount-coupon") translate([-35,-37.5,0]) mount_coupon();
else if(part=="mount-screw-check") mount_screw_collision();
else if(part=="mount-tool-check") mount_tool_collision();
else if(part=="joint-coupon-bottom") translate([-26.5,37,0]) joint_coupon_bottom();
...
else if(part=="fascia-insertion-check") fascia_insertion_collision();
else if(part=="lid-closing-check") lid_closing_collision();
...
// Reflect the case-side X mapping to match a rigid, component-up KiCad placement.
module bottom_shell_base() {
    mirror([1,0,0]) bottom_shell_board_coordinates();
}

// Reflect the case-side X mapping to match a rigid, component-up KiCad placement.
module carrier_witness() {
    mirror([1,0,0]) carrier_witness_board_coordinates();
}
...
module xz_profile(y,cx,cz,w,h,r,t=.001) {
    translate([cx,y,cz]) rotate([-90,0,0])
        linear_extrude(height=t) rounded_rect(w,h,r);
}
// Constant matched XZ guide profiles over the full9mm insertion depth.
// XZ polygons are converted into an X/-Z sketch and extruded along+Y.
module xz_prism(points,y0,y1) {
    translate([0,y0,0]) rotate([-90,0,0])
        linear_extrude(height=y1-y0) polygon([for(p=points) [p[0],-p[1]]]);
}
module guide_male() {
    xz_prism([[32.7,3.8],[36,3.8],[36,5.8],[32.7,9.1]],-52,-43);
}
module guide_cavity() {
    xz_prism([[32.7,3.8],[36.3,3.8],[36.3,41.8+sqrt(2)*.4-36.3],[32.7,41.8+sqrt(2)*.4-32.7]],-52.1,-43);
}
module probe_pocket_blank() {
    union() {
        bottom_shell_base();
        xz_profile(-52,0,11.65,64,20,4,9.5);
    }
}
module probe_pocket_cut() {
    hull() {
        xz_profile(-52.02,0,11.15,60,19,5.5);
        xz_profile(-44.5,0,11.65,50,12,3);
    }
    for(x=[-17,0,17]) {
        translate([x,-44.52,11.65]) rotate([-90,0,0]) cylinder(d=5.6,h=2.04);
        translate([x,-43.1,11.65]) rotate([-90,0,0]) cylinder(d1=5.6,d2=6.4,h=.62);
    }
}
module probe_fascia() {
    difference() {
        union() {
            intersection() {
                probe_pocket_blank();
                box_bounds(-33.1,33.1,-53,-42.5,0,27.4);
            }
            for(s=[-1,1]) scale([s,1,1]) {
                box_bounds(31,33.1,-52,-42.5,2.48,9.5);
                guide_male();
                box_bounds(29,31.3,-49.5,-45.5,18,25);
            }
            // Only the floor extends rearward; the probe plate remains fixed.
        ...
        probe_pocket_cut();
        for(x=[-21,21])
            box_bounds(x-6.3,x+6.3,-50.85,-48.25,20.78,27.42);
    }
}
module bottom_shell_with_guides() {
    difference() {
        union() {
            difference() {
                bottom_shell_base();
                box_bounds(-33.3,33.3,-53,-42,-.02,27.62);
            }
            intersection() {
                rounded_solid(case_width,case_length,8,corner_radius);
                for(s=[-1,1]) scale([s,1,1])
                    box_bounds(33.3,39,-52,-40.5,0,8);
            }
            // Keep raised rail roofs inside the original rounded exterior.
            intersection() {
                rounded_solid(case_width,case_length,10.6,corner_radius);
                for(s=[-1,1]) scale([s,1,1])
...
// This is an insertion lead-in, not a root fillet or tested strength feature.
module keeper_key(x) {
    // One continuous polyhedron avoids coincident internal cap triangles at the
    // exact chamfer-to-prism transition while preserving all nominal surfaces.
    polyhedron(points=[
        [x-5.6,-50.2,23],[x+5.6,-50.2,23],
        [x+5.6,-48.9,23],[x-5.6,-48.9,23],
        [x-6,-50.6,23.4],[x+6,-50.6,23.4],
        [x+6,-48.5,23.4],[x-6,-48.5,23.4],
        [x-6,-50.6,28.6],[x+6,-50.6,28.6],
        [x+6,-48.5,28.6],[x-6,-48.5,28.6]
...
module top_bezel() {
    union() {
        ...
        for(x=[-21,21]) {
            keeper_key(x);
            box_bounds(x-6,x+6,-51.5,-48.5,27.6,29.6);
        }
    }
}
module fascia_fit_collision() {
    intersection() {
        probe_fascia();
        union() {
            // Exclude only intentional support contact at Z3.8 and Y-43.
            intersection() {
                bottom_shell();
                union() {
                    box_bounds(-100,100,-100,100,-1,3.7999);
                    box_bounds(-100,100,-100,-43.0001,3.8001,60);
                    box_bounds(-100,100,-42.9999,100,3.8001,60);
                }
            }
            carrier_sweep(carrier_case_offset_y,carrier_case_offset_y,0,0);
            top_bezel();
...
// A nonconvex mesh is swept continuously by Minkowski sum with a tiny box.
// The box extends 0.00001 mm in X/Z; fixed supports are trimmed by 0.0001 mm
// only at deliberate contact planes, keeping the complete collision question.
module fascia_swept() {
    minkowski() {
        probe_fascia();
        translate([-.000005,-12,-.000005]) cube([.00001,12,.00001]);
    }
}
module fascia_insertion_collision() {
    intersection() {
        fascia_swept();
        union() {
            intersection() {
...
module lid_closing_collision() {
    // Height separation proves the retainer and all lid material above seam_z
    // cannot touch fixed solids: the highest fixed body is the bottom at27.6.
    // Conservatively sweep the XY projection of ALL original lid material below
    // that seam plus each new keeper key. The projection preserves lip gaps.
    assert(retainer_bottom_z-small_screw_head_height > seam_z);
    assert(carrier_bottom_z+check_pcb_thickness+16.2 < seam_z);
    intersection() {
        union() {
            translate([0,0,seam_z-locating_lip_height+.0001])
                linear_extrude(height=35+locating_lip_height)
                    projection(cut=false) intersection() {
                        top_bezel_base();
                        box_bounds(-100,100,-100,100,0,seam_z-.0001);
                    }
            for(x=[-21,21])
                box_bounds(x-6,x+6,-50.6,-48.5,23.0001,63.6);
        }
        union() {
            bottom_shell();
            probe_fascia();
...
// Right-side hand-held guide/key coupon. This includes no case screw clamp.
module joint_coupon_bottom() {
    intersection() { bottom_shell(); box_bounds(10,43,-52,-22,-.02,60); }
}
module joint_coupon_top() {
    intersection() { top_bezel(); box_bounds(10,43,-52,-22,-.02,60); }
}
module joint_coupon_fascia() {
    intersection() { probe_fascia(); box_bounds(10,43,-52,-22,-.02,60); }
}

module fascia_lift_collision() {
    // Approach the0.2mm upward stop within0.0001mm numerical margin.
    // At exactly0.2mm, the fascia top intentionally contacts seamZ27.6.
    // Tiny numerical trimming excludes only deliberate Y-43 guide end contact.
    intersection() {
        translate([0,0,.1999]) probe_fascia();
        union() {
            intersection() {
                bottom_shell();
                union() {
...
// r13 interior future-mount inserts, accessible from the outside bottom.
// 72x74mm pattern keeps an8mm heat-setting tool clear of wall/closure columns.
module bottom_shell() {
    difference() {
        union() {
            bottom_shell_with_guides();
            for(x=[-36,36],y=[-37,37])
                translate([x,y,2.5]) cylinder(d=10,h=5);
            for(sx=[-1,1],sy=[-1,1]) scale([sx,sy,1])
        ...
        // Runner stop remains intact through its maximum6.1mm height.
        for(s=[-1,1]) scale([s,1,1])
            box_bounds(33.3,39,-43,-40.5,7.5,8.02);
    }
}
...
module mount_tool_collision() {
    // Electronics/top removed. Tool envelope is8mm diameter up toZ35;
    // a larger iron barrel must remain above that declared working envelope.
    intersection() {
        for(x=[-36,36],y=[-37,37])
            translate([x,y,7.5001]) cylinder(d=8,h=35-7.5001);
        bottom_shell();
    }
}
module mount_coupon() {
    intersection() { bottom_shell(); box_bounds(27,43,27,48,-.02,10); }
}

module guide_capture_collision(y) {
    // Positive intersections are REQUIRED at both sides. The actual local
    // fascia rails, moved up0.7mm with the lid absent, must hit the rail roofs.
    // The obsolete open inner guide failed this behavior away from the face.
    intersection() {
        bottom_shell();
        translate([0,0,.7]) intersection() {
            probe_fascia();
            for(s=[-1,1]) scale([s,1,1])
                box_bounds(33.3,36.1,y-.1,y+.1,3.7,10);
        }
    }
}
