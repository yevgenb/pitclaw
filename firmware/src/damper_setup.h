...
public:
    ...
    void begin(const DamperCalibration& saved, uint16_t current, uint32_t now) {
        _state = {}; _state.endpoints = saved; _state.pulseUs = current;
    ...
    bool test(int percent, uint32_t now) {
        if (!_state.ready() || (percent != 0 && percent != 50 && percent != 100)) return false;
        _state.pulseUs = _state.endpoints.pulseAt(percent);
        _state.stopped = false; _lastMove = now; return true;
    }
