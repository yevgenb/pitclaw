...
assert summary['schematic_sha256']==sha(ROOT/(NAME+'.kicad_sch')),'Schematic changed since native checks'
assert summary['project_rules_sha256']==sha(ROOT/(NAME+'.kicad_pro')),'Project rules changed since native checks'
...
with tempfile.TemporaryDirectory(prefix='pitclaw-jlc-') as temp:
    ...
    assert sha(ROOT/(NAME+'.kicad_sch'))==summary['schematic_sha256'],'Schematic changed during export'
    assert sha(ROOT/(NAME+'.kicad_pro'))==summary['project_rules_sha256'],'Project rules changed during export'
